...
const int fill_vec1(std::shared_ptr<std::vector<MyInteger>> vec_ptr) {
    ...
    std::vector<MyInteger> vector;
    MyInteger myint(1);
    MyInteger myint2(2);
    vector.emplace_back(myint);
    vector.emplace_back(myint2);
    auto unique_holder = std::make_shared<std::vector<MyInteger>>(vector);
    auto t = unique_holder;
    auto f = t;
    ...
    const int total_past_owners = fill_vec2(unique_holder);
